...
const SideMenu = ({ activeMenu }) => {
  ...
  const handleLogout = async () => {
    try {
      const response = await axiosInstance.post("/auuth/sign-out")
    ...
    <div className='w-64 p-6 h-full flex flex-col lg:border-r lg:border-gray-200'>
      <div className='flex flex-col items-center mb-8'>
        <div className="w-20 h-20 rounded-full bg-gray-100 overflow-hidden mb-4 border-2 border-blue-200">
          <img
            src={currentUser?.profileImageUser || null}
        ...
        {currentUser?.role === "admin" && (
          <div className="bg-blue-100 text-shadow-blue-800 text-xs font-semibold px-2.5 py-0.5 rounded-full mb-2">
            Admin
          </div>
        )}
      ...
      <div className="flex-1 overflow-y-auto">
        {sideMenuData.map((item, index) => (
          <button
            key={`menu_${index}`}
            type="button"
            className={`w-full flex items-center gap-4 text-[15px] ${activeMenu === item.label ? "text-blue-500 bg-linear-to-right from-blue-50/40to-blue-100/50" : ""} py-3 px-6 mb-3 cursor-pointer`}
