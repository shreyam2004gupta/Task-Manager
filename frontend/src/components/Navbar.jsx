...
const Navbar = ({ activeMenu }) => {
  ...
  return (
    <div className="bg-white shadow-sm sticky top-0 z-10 p-4 flex items-center justify-between">
      ...
      <button className="p-2 rounded-md text-gray-700 hover:text-gray-900 hover:bg-gray-100 focus:outline-none focus:ring-2 
      focus:ring-inset focus:ring-indigo-500 transition-colors lg:hidden" onClick={()=>setOpenSideMenu((prev)=>!prev)}>
        {openSideMenu ? (<MdOutlineClose className="text-2xl"/>):(<MdMenu className='text-2xl'/>)}
      </button>
      </div>
